...
if __name__ == '__main__':
        ...
        # 修改测试环境为预生产
        f = IniUtil()
        f.update_value_of_opetion("Env", "env", "pre")
        ...
        ****************************************************************************************************************
        ...
        runner = HTMLTestRunner(stream=fp, title=u'b2b接口自动化测试报告--预生产', description=u'自动化测试结果如下：', verbosity=2)
        runner.run(suite())
        fp.close()
        ...
        # 第三个参数可填可不填
        send_report(report, to_addr=to_list, subject=u'b2b接口自动化测试报告')
